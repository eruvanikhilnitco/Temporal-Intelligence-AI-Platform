...
class DocumentReader:
    ...
    def parse_query(self, query: str) -> DocumentReadRequest:
        ...
        Steps:
          ...
          4. Find filename via exact then fuzzy matching (local + Qdrant)
        """
    ...
    def read(self, req: DocumentReadRequest) -> str:
        ...
        if req.line_number:
            ...
            return f"Line {req.line_number} of '{req.filename}':\n\n{line_text}"
    ...
    def _find_filename_in_query(self, query: str) -> Tuple[Optional[str], float]:
        ...
        for fname in available:
            ...
            # Require ≥2 matching words unless stem is a single word
    ...
    def _resolve_file(self, filename: str) -> Optional[Path]:
        return self._resolve_local_file(filename)

    def _list_available_files(self) -> List[str]:
        """Returns all known files: local upload_dir UNION Qdrant-indexed files."""
        local: List[str] = []
        try:
            local = sorted(
    ...
    def _get_qdrant_filenames(self) -> List[str]:
        """Get unique file_name values from Qdrant. Cached for 60 seconds."""
        now = time.time()
    ...
    def _read_from_qdrant(self, filename: str) -> Optional[str]:
        """Reconstruct document text from Qdrant chunks sorted by chunk_id/char_start."""
        try:
            from core.config import get_settings
            from qdrant_client import QdrantClient
            ...
            def sort_key(p):
                try:
                    return (int(p.payload.get("chunk_id", 0)), int(p.payload.get("char_start", 0)))
                except Exception:
                    return (0, 0)
